...
export class Visual implements IVisual {
    ...
    public update(options: VisualUpdateOptions) {
        ...
        let marginLeft = Visual.margin.left * (viewModel.YAxisFontSize / 10);
        let marginBottom = Visual.margin.bottom * (viewModel.XAxisFontSize / 10);
        let marginTop = Visual.margin.top;
        let marginRight = Visual.margin.right;

        let plotArea = {
            x: marginLeft,
            y: marginTop,
            width: options.viewport.width - (marginLeft + Visual.margin.right),
